...
import { LayoutDashboard, Rocket, Target, MessageSquare, GitHub, Aperture, Mic, X, Terminal, ClipboardList, Image, BookOpen, Heart, Globe, Compass, ShieldCheck, Users, Smartphone, Zap, GraduationCap, ShoppingBag, Code, GitPullRequest, CreditCard, Activity, Gift } from './IconComponents';
...
const Sidebar: React.FC<SidebarProps> = ({ currentView, onSetView, isMobileOpen, onMobileClose, repoStatus = 'Synced', onOpenMission, onShowWelcome }) => {
  ...
  return (
    <>
      ...
      <aside className={`fixed md:static inset-y-0 left-0 z-40 w-64 navy-gradient border-r border-brand-silver/10 transform transition-transform duration-300 ease-in-out flex flex-col ${isMobileOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
        <div className="h-[60px] flex items-center px-6 border-b border-brand-silver/20 relative navy-gradient">
            <div className="flex items-center gap-2 font-bold tracking-tight font-display">
                <span className="text-2xl metallic-text-3d">Ai</span>
                <span className="metallic-text">Collab<span className="text-brand-silver">Admin</span></span>
            </div>

...
const OpsControl: React.FC = () => {
  return (
    <div className="p-6 space-y-6 overflow-y-auto h-full bg-brand-void">
      ...
      {/* HAIKU AUDIT AGENTS - 24/7 Compliance Monitoring */}
      <div className="glass-card p-6 border-l-4 border-orange-500">
        <h2 className="text-xl font-bold text-orange-400 mb-4 flex items-center gap-2">
          <Shield className="w-6 h-6" /> 24/7 Haiku Audit Agents
        </h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {/* GPAA - Gospel Public Audit Agent */}
          <div className="p-4 bg-slate-800/50 rounded-lg border border-orange-500/30">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                <span className="w-3 h-3 rounded-full bg-green-500 animate-pulse"></span>
                <span className="font-bold text-white">GPAA</span>
              </div>
              <span className="px-2 py-0.5 text-xs rounded bg-green-900/30 text-green-400 border border-green-500/30">
                ONLINE
              </span>
            </div>
            <p className="text-xs text-slate-400 mb-2">Gospel-Public-Audit-Agent</p>
            <p className="text-sm text-slate-300">Monitors: Gospel V1.3 (60/30/10)</p>
            <div className="mt-3 grid grid-cols-3 gap-2 text-center">
              <div className="p-2 bg-black/30 rounded">
                <p className="text-xs text-slate-500">Private</p>
                <p className="text-sm font-bold text-orange-400">137</p>
              </div>
              <div className="p-2 bg-black/30 rounded">
                <p className="text-xs text-slate-500">Public</p>
                <p className="text-sm font-bold text-orange-400">72</p>
              </div>
              <div className="p-2 bg-black/30 rounded">
                <p className="text-xs text-slate-500">Enterprise</p>
                <p className="text-sm font-bold text-orange-400">32</p>
              </div>
            </div>
            <p className="text-xs text-slate-500 mt-2">Port: 3015 | Interval: 30min</p>
          </div>

          {/* ICB - Internal Compliance Bot */}
          <div className="p-4 bg-slate-800/50 rounded-lg border border-red-500/30">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                <span className="w-3 h-3 rounded-full bg-red-500 animate-pulse"></span>
                <span className="font-bold text-white">ICB</span>
              </div>
              <span className="px-2 py-0.5 text-xs rounded bg-red-900/30 text-red-400 border border-red-500/30">
                ALERT
              </span>
            </div>
            <p className="text-xs text-slate-400 mb-2">Internal-Compliance-Bot</p>
            <p className="text-sm text-slate-300">Monitors: Credentials, Wallets, Versions</p>
            <div className="mt-3 grid grid-cols-3 gap-2 text-center">
              <div className="p-2 bg-black/30 rounded">
                <p className="text-xs text-slate-500">Critical</p>
                <p className="text-sm font-bold text-red-400">34</p>
              </div>
              <div className="p-2 bg-black/30 rounded">
                <p className="text-xs text-slate-500">High</p>
                <p className="text-sm font-bold text-yellow-400">36</p>
              </div>
              <div className="p-2 bg-black/30 rounded">
                <p className="text-xs text-slate-500">Files</p>
                <p className="text-sm font-bold text-blue-400">702</p>
              </div>
            </div>
            <p className="text-xs text-slate-500 mt-2">Port: 3017 | Interval: 15min</p>
          </div>
        </div>
        <div className="mt-4 p-3 bg-black/30 rounded-lg border border-white/5">
          <p className="text-xs text-slate-400">
            <span className="text-green-400 font-bold">ALL 3 REPOS MONITORED:</span>{' '}
            AiCollabForTheKids (Private) | AiCollabForTheKids-Public | AiCollab-Enterprise
          </p>
        </div>
      </div>

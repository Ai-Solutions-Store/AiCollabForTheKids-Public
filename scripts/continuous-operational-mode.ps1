...
# Gospel Split (IMMUTABLE - V1.3)
$GOSPEL_SPLIT = @{
    charity = 60        # Kids get MORE
    Infrastructure = 30
    Founder = 10        # Founder took LESS
}
...
function Run-DailyAudit {
    ...
    Write-Log "Running Gospel Split audit (60/30/10 verification - Gospel V1.3)..." "AUDIT"
    try {
        ...
        if ($LASTEXITCODE -ne 0) {
            ...
            Write-Log "Gospel Split audit PASSED - 60/30/10 verified (Gospel V1.3)" "AUDIT"
        }
    } catch {
...
function Install-ScheduledTasks {
    ...
    try {
        ...
        Register-ScheduledTask -TaskName "FOR_THE_KIDS_Gospel_Audit" `
            ...
            -Description "Daily Gospel Split audit - 60/30/10 verification FOR THE KIDS (Gospel V1.3)" `
            -Force | Out-Null
...
Write-Host "  - Gospel Split Violation (60/30/10 V1.3)" -ForegroundColor White

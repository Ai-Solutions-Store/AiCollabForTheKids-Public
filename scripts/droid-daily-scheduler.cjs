...
// Configuration - Use live API (T5500 serves this)
const API_URL = 'https://api.aidoesitall.website/api/droid/generate-full-video';
...
function getRandomCategory() {
    // Valid categories: general, sports, business, technology, entertainment
    const weighted = ['technology', 'technology', 'business', 'business', 'sports', 'entertainment', 'entertainment', 'general'];
...
async function generateDailyVideo() {
    ...
    log('Mission: FOR THE KIDS - 60% to Verified Pediatric Charities Children\'s Hospitals');
    log('═══════════════════════════════════════════════════════════════');

    try {
        const response = await httpsPost(API_URL, { category });
...
log('║  Mission: FOR THE KIDS - 60% to Verified Pediatric Charities Children\'s          ║');
log('╚═══════════════════════════════════════════════════════════════╝');
...
log('');
log('Manual trigger: POST https://api.aidoesitall.website/api/droid/generate-full-video');
log('');
